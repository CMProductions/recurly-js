...
export class Request {
  ...
  xhr ({ method, url, data }) {
    return new Promise((resolve, reject) => {
      ...
      req.open(method, url);
      req.timeout = timeout;
      req.ontimeout = () => reject(this.recurly.error('api-timeout'));
      req.onerror = () => reject(this.recurly.error('api-error'));
      req.onprogress = NOOP;
      req.onload = function () {
        let res;
        try {
          ...
          return reject(this.recurly.error('api-error', {
        ...
        if (res && res.error) {
          reject(this.recurly.error('api-error', res.error));
        } else {
          resolve(res);
        }
      };
