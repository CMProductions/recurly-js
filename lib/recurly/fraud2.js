...
export function factory (options) {
  return new Fraud2(Object.assign({}, options, { recurly: this }));
}

export class Fraud2 extends Emitter {
  constructor (options) {
    super();
    ...
    this.kountProvider     = providers.find(provider => provider.provider === 'kount') || { dataCollector: false, form: undefined };
    this.braintreeProvider = providers.find(provider => provider.provider === 'braintree') || {};
    this.litleProvider     = providers.find(provider => provider.provider === 'litle') || {};

    if (this.kountProvider && !!this.kountProvider.dataCollector) {
      this.recurly.ready(this.attachDataCollector.bind(this));
    }
  }
  ...
  params (data) {
    ...
    if (this.kountProvider.dataCollector && data.fraud_session_id) {
      fraudParams.push({
        processor: 'kount',
    ...
    if (this.litleProvider.sessionId) {
      fraudParams.push({
    ...
    if (this.braintreeProvider.deviceData) {
      fraudParams.push({
        processor: 'braintree',
   ...
   */
  attachDataCollector () {
    if (this.kountProvider.dataCollectorInitiated) return;
    this.kountProvider.dataCollectorInitiated = true;
    ...
    this.recurly.request.get({
      route: '/risk/info',
        ...
        profiles.forEach(profile => {
          ...
          if (processor === 'kount') {
            const configuredForm = this.kountProvider.form;
